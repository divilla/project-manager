...
Program construction is centralized at the complete-program boundary. Production startup resolves
the repository and supplies the real UUID generator and agent runner. Program integration tests use
the same Bubble Tea loop while supplying a temporary repository root, deterministic UUID generator,
controlled context, and fake agent runner; keyboard input, rendering, HTTP calls, and editor
subprocess suspension and resume remain production paths.
...
- `parse recovery`: the edited idea has an invalid title, Types line, opening layout, or type slug and is waiting for
...
When `/new-change` starts from `ChangesListState`, `mch` first requires a valid numeric current project ID, then generates one UUIDv7 and creates `.mch/tmp/<ref-uuid>/idea` beneath the Git repository root. It creates blank `input.md` and `output.md` in that directory before opening the editor on `output.md`; `input.md` remains the blank comparison baseline. UUID generation or initialization failure prevents editor and API execution, removes any partial workspace created for that UUID, and reports a recoverable error.

After the editor exits, `mch` compares `input.md` and `output.md` byte for byte. Equal files cancel the flow, remove the generated UUID workspace, and return to `ChangesListScreen` without calling the backend. Changed output proceeds directly to `Create Change?` without artifact validation.

`Create Change?` offers Yes and No choices. No removes the generated UUID workspace and returns to `ChangesListScreen` without calling the backend. Yes re-reads `output.md`, requires a non-blank H1 title and non-metadata Idea body, and calls `POST /api/v1/change/create` only after validation succeeds. `Types:` is optional: omitting it sends no type update, while including it always calls `POST /api/v1/change/update-change-types`. An empty value sends an empty string array; a non-empty value treats pipes and whitespace as separators, then removes every `[^A-Za-z\-_]` character from each value and discards empty values. Artifact type processing does not load the options catalog or reject unsupported values; the backend silently filters submitted arrays against its available types and uses the filtered array as the Change's complete new type set. `Epic:` is ordinary artifact text rather than metadata, so this flow does not list, validate, extract, or update an Epic. Validation errors keep both files available for correction and send no create request. A successful create sends numeric `project_id`, the generated `ref_uuid`, parsed title, and complete content as `idea`. Its response immediately becomes the current Change in `ChangeDetailsState`. Included `Types:` metadata then causes `POST /api/v1/change/update-change-types`, including when its parsed array is empty, and `mch` uses the complete Change returned by that update without a separate reload. Omitted metadata sends no type-update request. A create API failure keeps both files for retry and does not present a Change. After successful creation, `mch` copies `output.md` over `input.md` and enters `RewriteIdeaState`. If that copy or the post-create type update fails, the error flow remains anchored to the persisted Change: `/fix` edits and updates it, while unchanged editor exit and `/cancel` retain the workspace and return to `ChangeDetailsState` without another create request. Successful rewrite reloads the created Change before opening `ChangeDetailsState`. Existing-Change operations never invoke the initializer or truncate existing workspace files.

`RewriteIdeaState` resolves the repository root with `git rev-parse --show-toplevel` and runs Codex from that root against the Change's UUID-scoped `idea` workspace. The internal state name is retained while `idea-write`, `spec-write`, and `pr-write` run. `AgentRunningScreen` shows an animated loader with elapsed seconds while Codex runs and displays available Codex command output above the prompt in a human-readable JSON format. A new session ID is extracted from `thread_id` on the first `thread.started` JSON event, with older shapes supported as fallbacks, and is saved to the idea workspace. Existing artifact writes resume `session-id`. Missing session ID or a failed final result shows a verbose recoverable error and keeps the formatted command output visible. Successful output is saved through the matching Idea, Spec, or PR endpoint with `agent_edit` set to true, followed by `POST /api/v1/change/get` and `ChangeDetailsState`. Backend failures show a recoverable error and must not display local-only success state.
...
Enter or Return on editable detail rows starts the focused edit flow for that field. `ChangeDetailsScreen` footer hint is `<ctrl+n> new testcase | <return> edit | <space> toggle | <del> delete | <ctrl+ins> copy | </> command`; Ctrl+N behaves the same as selecting `/new-testcase` from the command menu. Phase and Epic open selectors with the current value highlighted; Phase selector options render with a leading `-`, and Epic appends `@none` to clear the epic. Types opens a toggle selector in backend option order and persists changed selections through `POST /api/v1/change/update-change-types` followed by `POST /api/v1/change/get`; canceling never persists pending toggles. Title opens `ChangeUpdateState`, and PR URL remains a focused text save. Idea, Spec, and PR first reload the Change, prepare `.mch/tmp/<ref_uuid>/idea/output.md` from backend text, copy it to `input.md`, preserve `session-id` and logs, and open the persistent output file. Unchanged output returns to details without mutation. Changed output saves with `agent_edit=false`, applies present `Types:`, reloads, copies the saved `output.md` to `input.md`, and only then runs the matching `idea-write`, `spec-write`, or `pr-write` prompt with `MCH_STAGE=idea`. Agent output is saved with `agent_edit=true` and reloaded again. `/edit-spec` uses this same Spec path. `/new-testcase` and test case row actions continue to use their focused create, update, toggle, and delete endpoints. Backend failures show a recoverable error and must not display local-only success state.

`ChangeCreateState` renders `ChangeCreateScreen` as the header context and exposes `/save` and `/cancel` for non-agent create paths. Save parses an idea whose first line is a non-blank H1, then creates the Change through `POST /api/v1/change/create` with numeric `project_id`, parsed title, and idea. It does not generate or submit `ref_uuid`; the backend generates that identity for this path. It must not submit `ref`, `slug`, project reference counters, `change_phase`, `change_types`, `epic_id`, `spec`, `pr`, `pr_url`, `agent_edit`, or `open`. Successful create reloads the created Change from the backend before opening details. Validation or backend failures keep the user in a recoverable create state with the edited content available to fix.

`ChangeUpdateState` renders `ChangeUpdateScreen` as the header context and exposes `/save` and `/cancel` for spec edits. Opening `/edit-spec` must open the external editor with the current title, spec string, optional types, and optional epic. Save parses a non-blank `# <title>`, then persists changed title, spec, included types, and epic through `POST /api/v1/change/update-title`, `POST /api/v1/change/update-spec` with `agent_edit` set to false, `POST /api/v1/change/update-change-types`, and `POST /api/v1/change/update-epic`. Omitted type metadata leaves types unchanged; included type metadata always calls the type endpoint, even when its parsed array is empty or matches the previously loaded types. Successful update reloads the Change through `POST /api/v1/change/get` before refreshing details. Apart from the presence-driven type update, unchanged fields do not call their focused endpoints.

Change create and update allow no selected types. Interactive selectors use the startup-loaded result from `POST /api/v1/options/change-types-list`, but Idea, Spec, and PR metadata bypasses that client-side catalog check and relies on the backend to discard unsupported submitted values without a validation error. Epic names are resolved from `POST /api/v1/epic/list` for the current project. An unparsable title or blank idea prevents any create or update API call. Omitted artifact type metadata leaves existing types unchanged; explicit empty metadata persists an empty `change_types` array.
...
`mch` resolves the Git repository root at startup, then loads committed repository config from `.mch/config.yaml`. The file owns `backend_url` and numeric `project_id`; `project_id` is shared branch-scoped state for everyone using that repository branch. `.mch` is project configuration, not a secrets or user-local directory, and must remain eligible to be committed.

Runtime planning workspaces use the shared `TempDir` constant `.mch/tmp`, resolved beneath the Git repository root. A legacy `temp_dir` key is ignored and is not loaded, displayed, persisted, or migrated. Automated tests and verification commands may use disposable system temporary directories, including `t.TempDir()` paths and temporary build output paths, but those paths are test infrastructure rather than runtime planning defaults.
...
Flow profile management, loading, validation, and read-only rendering do not execute workflow behavior. `mch` does not launch configured hooks or default Makefile targets. Artifact-write processes override inherited values with `MCH_DEFAULT_DIR=.mch/default`, `MCH_TEMP_DIR=.mch/tmp`, the created or reloaded Change UUID as `MCH_REF_UUID`, and `MCH_STAGE=idea`. Idea, Spec, and PR writing all use that stage; artifact kind selects `idea-write`, `spec-write`, or `pr-write`, not another workspace. The CLI resolves these values without consulting Flow profile data. Selecting later stages from an active Flow Step remains deferred.
...
`spec-write.md` reads the generated Change type catalog for semantic type selection and emits a populated `Types:` line. The canonical structure prompt accepts omitted, empty, and populated `Types:` metadata and validates only placement and pipe-delimited syntax. Other prompt Markdown must not call backend APIs.

The CLI must not implement Flow profile selection, config overrides, fallback layering, `.mch/local`, or compatibility loading from old config locations. Missing, unreadable, or malformed `.mch/config.yaml`, `.mch/default/flow.yaml`, or `.mch/default/help.yaml` must produce a verbose startup config error without silently substituting another source. The error must include enough path-specific detail for the user to identify and fix the failing file, then `mch` exits without starting the interactive app.

`/config` renders a read-only view from resolved in-memory configuration. It must show enough data to verify the active config source, including repository root, config path, `backend_url`, `project_id`, active Flow directory, Flow metadata, Step data, hook command strings, stage modes, task statuses, and task steps. It must not display `temp_dir`. Opening or leaving `/config` must not call backend APIs, read raw YAML for rendering, execute Flow hooks, or save any config file.
...
Planning tests should cover the ordered `/new-change` UUID, workspace, blank-file, editor, comparison, confirmation, validation, and create steps. They should verify equal-file and No-choice cancellation with workspace removal; missing-title and blank-body failures after Yes with retained files and no request; optional, empty, and unsupported `Types:` values without client-side type validation; `Epic:` as ordinary artifact text; Yes-choice `ref_uuid` submission; initialization cleanup; backend failure retry state; and protection of existing Change workspaces. Editor, backend, and external-process dependencies should be faked.

Existing artifact-write tests should cover Idea, Spec, and PR operation selection; reloading `ref_uuid`; shared `idea` workspace and session reuse; backend and post-user-save output-to-input preparation; unchanged cancellation; ordered user and agent saves; type processing; `MCH_STAGE=idea`; and final Change reload. Program integration coverage drives the complete CLI with controlled key input and rendered output, an HTTP test backend, a subprocess-backed fake editor, and an injected agent runner. It does not call model methods or synthesize asynchronous completion messages.

New-Change agent-process tests should verify `MCH_DEFAULT_DIR=.mch/default`, `MCH_TEMP_DIR=.mch/tmp`, the generated Change UUID as `MCH_REF_UUID`, and `MCH_STAGE=idea`, including inherited-value replacement, command-failure propagation, and no hook execution during config loading or `/config` rendering.

Config tests should cover repository-root resolution from nested directories, `.mch/config.yaml` parsing without `temp_dir`, ignored legacy `temp_dir` values, `.mch/default` Flow parsing, fixed `TempDir` use, `/config` command routing, read-only config rendering without `temp_dir`, no-selection `project_id` states, saved project ID persistence, and missing or malformed config files with no fallback to old config locations.
