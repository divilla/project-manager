...
`mch` always loads the active Flow profile from `.mch/default`. It parses `flow.yaml` and `help.yaml` into typed in-memory config, including Flow metadata, ordered Step slugs, Step help text, Step mode, prompt paths, exact `entry`, `exec`, and `exit` hook command strings, stage modes, task statuses, and task steps. The supported default Flow Step order is `idea`, `spec`, `ready`, `docs`, `code`, `polish`, `pr`, `review`, `fix`, `sync`, `merge`, `stage`, and `master`; Step slugs must be non-empty and unique. Step mode in `flow.yaml` is mandatory and must be `skip`, `prompt`, or `exec`. Flow help option groups are configuration data: listed stage modes, task statuses, and task steps only require non-empty slugs, and missing groups, empty groups, custom slugs, and reordered slugs are valid.
