...
`ChangeDetailsState` renders `ChangeDetailsScreen - Title: Change Details` from the backend Change response. Details render as a scrollable two-column table with no header: labels are right-aligned in the first column and values render in the second column. Rows appear in this order: Ref, Slug, Phase, Epic, Types, Title, Requirement, Pull Request, PR URL, Agent Edit, Complete, Open, Created, and Modified. All rows are selectable. Phase renders with the same phase colors used by `ChangesListState`; the title value renders bright white. Requirement and Pull Request values are rendered from `body` and `pr_body` and truncated to 15 visible lines plus `...` when longer. Dividers separate Types from Title, Title from Requirement, Requirement from Pull Request, and Pull Request from PR URL. Created and Modified render as `YYYY-MM-DD HH.MM`.

Enter or Return on editable detail rows starts the focused edit flow for that field. Phase and Epic open selectors with the current value highlighted; Epic appends `@none` to clear the epic. Types opens a toggle selector in backend option order, using `+` for unselected types and `-` for selected types; saving types sends the sorted type slug list. Title opens `ChangeUpdateState` with a title prompt. Requirement, Pull Request, and PR URL use focused editors and persist through `POST /api/v1/change/update-body`, `POST /api/v1/change/update-pr-body`, and `POST /api/v1/change/update-pr-url`. Successful focused saves reload the Change through `POST /api/v1/change/get`, return to `ChangeDetailsState`, and keep the same detail row selected. `/edit` still opens full Change body markdown editing; editor save repopulates title, types, epic, and `body` from markdown metadata.

`ChangeCreateState` renders `ChangeCreateScreen - Title: New Change` and exposes `/save` and `/cancel`. Opening Change create must open the external editor; the only editable input is the Change markdown body. Save parses title, types, and optional epic from that body, validates the extracted fields, then creates the Change through `POST /api/v1/change/create` with numeric `project_id`, title, full preserved `body`, one or more `change_types`, and optional `epic_id`. Successful create opens details for the created Change. Validation or backend failures keep the user in a recoverable create state with the editor content available to fix.
...
Changes filters are list-local. `/phase-filter` loads options from `POST /api/v1/options/change-phases-list`, `/type-filter` loads options from `POST /api/v1/options/change-types-list`, and `/epic-filter` loads epics from `POST /api/v1/epic/list` with numeric `project_id`. Each filter overlay remains on `ChangesListState`, keeps the list title visible, and appends `/clear` as the final option to clear only that filter field. `/find-filter` applies text filtering to the loaded list by title, `ref`, `slug`, phase, type, epic, and loaded body text when present. `/clear-filters` clears phase, type, epic, and find filters and restores the unfiltered loaded list. A filter with no matches renders a no-results state.
