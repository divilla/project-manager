...
Create payloads require numeric `project_id`, non-blank `title`, and non-blank `idea`, and may include optional `ref_uuid`. An omitted or null `ref_uuid` causes the backend to generate the identity; a supplied value is preserved. Clients must not send `ref`, `slug`, `change_phase`, `change_types`, `epic_id`, `spec`, `pr`, `pr_url`, `agent_edit`, or `open` in the minimal create payload. Optional artifact fields default to empty strings, `epic_id` defaults to null, `change_types` defaults to an empty array, and `change_phase` defaults to `backlog`.

Change responses include `id`, resolved `ref_uuid`, optional project-scoped `ref`, optional `slug`, aggregate fields such as `done_tc`, `total_tc`, and `completed`, timestamps where appropriate, `agent_edit`, and `open`. Change list, get, create, Flow assignment, and focused update responses return `ref_uuid`, `ref`, and `slug` when returning a change object. `ref_uuid` is accepted only as an optional create input and is otherwise read-only; clients must not submit it in update payloads. `ref` and `slug` use null or the documented unassigned representation until Flow assignment. Compatibility for clients that omit `ref_uuid` is provided by the API contract and does not require frontend workflow or presentation changes.
...
Focused update endpoints identify the Change by numeric `id`, mutate only the named field, and return the refreshed Change. They must work before and after Flow assignment and must preserve the existing `ref` unless Flow assignment is called. `POST /api/v1/change/update-idea`, `POST /api/v1/change/update-spec`, and `POST /api/v1/change/update-pr` require non-empty string artifact fields and a boolean `agent_edit` that records whether the saved artifact was agent-produced. User-driven artifact edits send `agent_edit: false`; agent-produced artifact saves send `agent_edit: true`. `POST /api/v1/change/update-pr-url` requires a non-empty absolute `http` or `https` URL string. Empty artifact strings are the read-side no-value state for artifacts that have not been submitted yet, not a valid focused-update payload. Clients that update Change title, `idea`, `spec`, `pr`, `pr_url`, `open`, `change_types`, or `epic_id` should use the matching focused endpoint rather than submitting a broad edit payload.

`POST /api/v1/change/update-change-types` accepts any string array. The backend intersects the submitted values with the currently available Change type slugs, silently discards unsupported values, and replaces the Change's complete type set with the filtered array. An empty submitted array or an array with no supported values clears the Change's types. Unsupported values do not produce a validation error, so clients processing Idea, Spec, or PR metadata do not need to call `POST /api/v1/options/change-types-list` before this update. After an artifact-processing client performs this update, it reloads the Change through `POST /api/v1/change/get` rather than treating its submitted array as authoritative backend state.
...
Change phase and type option slugs come from the database. Clients that display or select allowed values must not hardcode them; they retrieve them from `POST /api/v1/options/change-phases-list` and `POST /api/v1/options/change-types-list`. Artifact metadata processors may instead submit their string arrays directly to `POST /api/v1/change/update-change-types` and rely on its backend filtering contract. Change phase and type options are ordered by `priority` and then `slug`. Phase options may include display color metadata; clients should use it when present and fall back to neutral grey when absent. The canonical phase color metadata uses Lip Gloss color numbers: backlog `15`, staging `12`, progress `10`, rejected `9`, production `13`, and review `11`.
