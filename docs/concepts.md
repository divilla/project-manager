...
- `slug`: stable backend-owned identifier generated by Change insert persistence.
- `title`: short human-readable name.
- `body`: markdown-capable description of the requirement or implementation intent.
...
Users and clients cannot set or edit `ref` or `slug`. The backend assigns them through Change insert persistence and returns them on change responses.
