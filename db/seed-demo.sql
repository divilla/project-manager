...
join (values
    ...
    ('Completeness Engine', 'review', array['feature'], 'Create binary requirement', 'Attach a concrete definition of done to a change.'),
    ('Completeness Engine', 'progress', array['feature'], 'Recalculate change counters', 'Count completed and total requirements for a change.')
) as seed(epic_name, phase, types, title, body)
    on true
join public.epic e on e.project_id = p.id and e.name = seed.epic_name
where p.name = 'demo1';

insert into public.requirement (change_id, definition, done)
select
    c.id,
    seed.definition,
    seed.done
...
join (values
    ...
    ('Create binary requirement', 'Requirement create returns recalculated change data.', false),
    ('Recalculate change counters', 'Done toggles update change completeness.', false)
) as seed(title, definition, done)
...
begin
    for _change in select id from public.change loop
        call public.sp_change_requirement_recalculate(_change.id);
    end loop;
end;
$$;
