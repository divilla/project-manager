...
(
    ...
    done_tc           smallint                 default 0               not null,
    total_tc          smallint                 default 0               not null,
    completed         smallint generated always as (COALESCE(((done_tc / NULLIF(total_tc, 0)))::integer, 0)) virtual,
    created           timestamp with time zone default now()           not null,
    modified          timestamp with time zone default now()           not null
);
...
create function public.fn_change_insert(_project_id bigint, _change_types text[], _epic_id bigint, _title text, _requirement_body text)
    returns bigint
    language plpgsql
as
$$
