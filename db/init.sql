...
(
    ...
    total_tc     smallint                 default 0               not null,
    completed    smallint generated always as
        (COALESCE(((((100 * done_tc) / NULLIF(total_tc, 0)))::smallint)::integer, 0)) stored,
    created      timestamp with time zone default now()           not null,
    modified     timestamp with time zone default now()           not null
);
...
create procedure public.sp_change_ref_update(_change_id bigint)
    language plpgsql
as
$$
declare
    ...
    _title text;
begin
    select project_id, ref, slug, title into _project_id, _ref, _slug, _title from change where id=_change_id for update;
