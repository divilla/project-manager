# Feature 07: Project Management and Active Project Selection

## 1. Purpose

Make projects a first-class application concept instead of hiding selection inside task-board behavior.

Users should be able to create, rename, inspect, select, and safely retire projects. A project is the context for dashboards, planning, task creation, and requirement management, so the top menu needs a durable active-project selector on its left side.
...
- top-menu left-side active-project selector
- Projects route for project CRUD only
- Tasks route for task-board workflows
- Home, Planning, and task-board flows that need the active project ID
- frontend tests for project CRUD and active-project selection

Out of scope:

- task CRUD redesign beyond moving the existing task board to `/tasks`
- dashboard metrics beyond consuming the active project ID
- planning copilot implementation beyond receiving the active project ID
...
- keep active-project selection valid after changes

Task-board behavior belongs on `/tasks`.

The page should make the safe-deletion rule visible. A project with tasks can show a disabled delete action, a tooltip, or a confirmation dialog that explains the task dependency. The backend remains authoritative even if the frontend cannot determine task usage locally.
...
## 5. Active Project Selector

Add an active-project selector to the left side of the top menu, near the application title and before the primary route tabs.
...
- expose the active project ID to pages and feature workflows
- update the active project when the user selects a different project
- reload the active project if the persisted ID no longer exists
...
3. If no projects exist, clear the active project and route the user to the Projects page.
4. If the active project is renamed, keep it selected.
5. If the active project is deleted, select the remaining project with the lowest ID.
...
The exact file names can follow existing conventions, but the active-project state should be feature-owned and reusable from the app shell, Home, Planning, and Projects workflows.

## 7. Integration Rules

Pages and workflows that operate inside a project must consume the active project ID instead of each owning a separate project selector.

Home:

- use active project ID for dashboard loading
- show an empty state when no project exists

Planning:

- use active project ID when committing generated tasks
- block commit actions until a project exists

Projects:

- provide project CRUD only
- keep the top-menu active project selector valid after project CRUD changes

Tasks:

- use the active project ID to choose which task board context is shown
- include the existing task create, board, detail, and requirement workflows

Task creation:

- send the active project ID as `project_id`
- block task creation when no active project exists
...
- compact select in the left side of the top menu
...
- use a confirmation step for deletable projects
...
6. Add the top-menu left-side project selector.
...
9. Update Home, Planning, and task-board flows to consume the active project ID.
...
- The top menu includes a left-side active-project selector.
- Active project selection persists across reloads.
...
- Task creation and other project-scoped workflows use the active project ID.
- Frontend tests cover the active-project selection rules.
