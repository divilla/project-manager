...
2. This handler must:
   ...
   - Return the validated list of proposed tasks and requirements arrays back to the client. Phase and type values must be validated against `task_phase` and `task_type` rows loaded from the existing database.
...
2. Structure the screen into two vertical halves:
   ...
   - **Right Half (Results Panel):** An empty state card explaining how the planner operates. Once a response is received, render the LLM suggestions dynamically using a structured collapsible list grouped by database-provided phases.
...
3. On the Go backend, process this batch operation within a secure SQL transaction:
   - Insert each task row mapping to an existing phase and type option without modifying reference tables.
   - Read the generated task IDs.
   - Insert all nested requirement rows pointing back to their newly created parent tasks.
   - If the AI commit updates or deletes existing tasks/requirements instead of only creating new rows, copy the current rows into `task_history` or `requirement_history` before changing them.
   - For AI-driven deletes, write history rows with `deleted = true`.
...
- [ ] **Reference Data Safety:** AI output never creates or changes `task_phase` or `task_type` options.
- [ ] **AI History Safety:** Any AI-driven task/requirement update or delete writes the previous current version to the matching history table.
