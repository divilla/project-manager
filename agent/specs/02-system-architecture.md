...
- **Approach:** Standard relational schema tracking projects, phases, tasks, requirements (DoD items), and AI-prompt interactions.
...
3. **Backend Action:** 
   ...
   - It updates the task state in the DB.
4. **Aggregation:**
...
3. **Backend Processing:**
   ...
   - It compiles a structured system prompt, injecting guidelines on breaking down tasks into 3-5 phase-grouped, verifiable sub-requirements.
...
1. **Strict Decoupling:** Keep frontend and backend strictly separated. Do not embed frontend files into Go binary assets during the prototype phase; run separate watch servers (Vite on port `9000` / Go on port `8080`) to ensure fast feedback loops.
