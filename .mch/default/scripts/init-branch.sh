...
change_slug="${1}"
if [[ ! "${change_slug}" =~ ^[A-Za-z0-9][A-Za-z0-9._-]*$ ]]; then
  printf 'invalid change slug: %s\n' "${change_slug}" >&2
  exit 1
fi
if [[ ! "${change_slug}" =~ ^([0-9]+)- ]]; then
  printf 'change slug must start with a zero-padded numeric ref: %s\n' "${change_slug}" >&2
  exit 1
fi
change_ref="${BASH_REMATCH[1]}"
