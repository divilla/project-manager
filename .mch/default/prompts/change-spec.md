...
Use `agent/ideas/<change-name>.md` to write a completed, implementation-ready Change specification to `specs/<change-name>.md` like a very senior dev. If that Change spec already exists, overwrite it intentionally.

Only write `specs/<change-name>.md`. Do not edit any other file, including `AGENTS.md`. Do not run migrations, mutate databases, edit files under `db/**`, or perform unrelated cleanup.
...
6. Write the completed Change specification to `specs/<change-name>.md`.
7. Add and commit only `specs/<change-name>.md`; do not include unrelated staged or unstaged changes.
8. Use this commit message: `Write spec for <change-name> by agent`.
...
- the branch name does not match `^change([0-9]+-[a-z0-9-]+)$`
- `agent/ideas/<change-name>.md` does not exist or cannot be read
